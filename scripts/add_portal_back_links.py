#!/usr/bin/env python3
"""Add a consistent Back to Portal link to training modules.

This script updates only HTML files in Introduction, Intermediate, and Advanced.
It is idempotent: running it again will not add duplicate links or duplicate CSS.
"""
...
MODULE_DIRS = ["Introduction", "Intermediate", "Advanced"]
...
def insert_link(text: str) -> str:
    ...
    # Introduction-style modules: header has top-controls container.
    if '<div class="top-controls">' in text:
        return text.replace('<div class="top-controls">', '<div class="top-controls">\n    ' + PORTAL_LINK, 1)

    # Advanced/newer modules: topbar contains logo block and a closing topbar div.
    topbar_close = "  </div>\n</div>\n\n<div class=\"nav-bar\""
...
def main() -> int:
    ...
    for dirname in MODULE_DIRS:
        for html_file in sorted((root / dirname).glob("*.html")):
